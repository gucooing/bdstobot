...
func main() {
	s.AddHandler(func(s *discordgo.Session, r *discordgo.Ready) {
		log.Printf("登录bot: %v#%v", s.State.User.Username, s.State.User.Discriminator)
	})
	err := s.Open()
	if err != nil {
		log.Fatalf("bot无法连接到discord: %v", err)
	}
	log.Println("注册命令中...")
	...
	for i, v := range commands {
		...
		if err != nil {
			log.Panicf("无法注册 '%v' 命令: %v", v.Name, err)
		}
	...
	log.Println("按下 Ctrl+C 退出")
	<-stop

	if *RemoveCommands {
		log.Println("注销命令中 请稍等...")

		for _, v := range registeredCommands {
			err := s.ApplicationCommandDelete(s.State.User.ID, *GuildID, v.ID)
			if err != nil {
				log.Panicf("无法注销 '%v' 命令: %v", v.Name, err)
			}
		}
	}
	log.Println("已完全退出.")
